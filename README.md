...
|- **Fresh Install Guard Fix** (2026-06-30): Fixed installation being blocked when `C:\ProgramData\OSGuard` exists but the installed script `OS_Lockdown.ps1` is missing (e.g., after a Windows reinstall or manual directory cleanup). The install gate in `Install-Persistence` and the menu option `[3]` availability check now verify the actual script file (`$InstallScript`) rather than just the directory (`$InstallDir`), so a fresh install is possible even if the directory was auto-created by `Write-Log` or other standalone operations.
|- **Auto-Enable ExecutionPolicy** (2026-06-30): When running elevated, the script automatically sets `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser -Force` if the current policy is `Restricted`, `AllSigned`, or `Undefined`. This removes the manual prerequisite step and ensures the script and future PowerShell runs work out of the box.
|- **Apply-EdgePolicies Get-Member Fix** (2026-06-30): Fixed `Get-Member` null-object error in `Apply-EdgePolicies` when the `URLBlocklist` registry key has no properties yet. The pipeline is now guarded so `Get-Member` only runs when the object exists, preventing `Set-StrictMode` from throwing.
